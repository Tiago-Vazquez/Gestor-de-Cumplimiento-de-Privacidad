...
export default function FindingsPage() {
  ...
  return <section className="page-in mx-auto max-w-[1440px] px-5 py-8 md:px-9 md:py-10">
    ...
    <div className="mb-5 flex flex-col gap-3 rounded-2xl border border-card-border bg-card p-3 shadow-[var(--shadow-card)] md:flex-row md:items-center">
      ...
      <div className="flex items-center gap-2"><Filter size={14} className="ml-1 text-muted-foreground" /><select value={severity} onChange={(event) => setSeverity(event.target.value as typeof severity)} className="h-10 rounded-lg border border-border bg-background px-3 text-xs font-semibold outline-none focus:border-primary" data-testid="select-severity"><option value="">Severidad</option><option value="critical">Crítico</option><option value="high">Alto</option><option value="medium">Medio</option><option value="low">Bajo</option></select><select value={status} onChange={(event) => setStatus(event.target.value as typeof status)} className="h-10 rounded-lg border border-border bg-background px-3 text-xs font-semibold outline-none focus:border-primary" data-testid="select-status-filter"><option value="">Estado</option><option value="open">Abierto</option><option value="in_review">En revisión</option><option value="resolved">Resuelto</option></select></div>
    </div>
    {findingsQuery.isLoading ? <div className="space-y-4" data-testid="findings-loading"><FindingsSkeleton /></div> : findingsQuery.isError ? <div className="rounded-2xl border border-[#edc5bd] bg-[#fff8f6] p-10 text-center" data-testid="list-error"><CircleX className="mx-auto mb-3 text-[#bf5a4e]" /><p className="font-display font-bold">No pudimos cargar los hallazgos.</p><button onClick={() => findingsQuery.refetch()} className="mt-4 text-xs font-bold text-primary underline" data-testid="button-retry-findings">Reintentar conexión</button></div> : findings.length === 0 ? <div className="rounded-2xl border border-dashed border-border bg-card px-6 py-16 text-center" data-testid="empty-state"><ShieldAlert className="mx-auto mb-3 text-muted-foreground/45" size={30} /><p className="font-display text-lg font-bold">No hay hallazgos con estos filtros</p><p className="mt-1 text-sm text-muted-foreground">Prueba con otra combinación o limpia la búsqueda.</p><button onClick={() => { setStatus(''); setSeverity(''); setSearch(''); }} className="mt-5 rounded-lg border border-border bg-background px-4 py-2 text-xs font-bold hover:border-primary" data-testid="button-clear-filters">Limpiar filtros</button></div> : <div className="overflow-hidden rounded-2xl border border-card-border bg-card shadow-[var(--shadow-card)]"><div className="hidden grid-cols-[1.6fr_.8fr_.75fr_.65fr_.7fr_24px] gap-4 border-b border-border bg-[#f8fafb] px-5 py-3 font-mono text-[10px] uppercase tracking-[0.12em] text-muted-foreground md:grid"><span>Hallazgo</span><span>Fuente</span><span>Severidad</span><span>Registros</span><span>Estado</span><span /></div>{findings.map((finding) => <button onClick={() => setSelected(finding)} key={finding.id} className="grid w-full grid-cols-1 gap-3 border-b border-border/70 px-5 py-4 text-left transition-colors last:border-0 hover:bg-[#f5faf8] md:grid-cols-[1.6fr_.8fr_.75fr_.65fr_.7fr_24px] md:items-center md:gap-4" data-testid={`row-finding-${finding.id}`} data-open-button={finding.id}><div className="min-w-0"><p className="truncate text-sm font-bold">{finding.title}</p><p className="mt-1 flex items-center gap-1.5 text-xs text-muted-foreground"><span className="font-mono">{dataLabels[finding.dataType]}</span><span>·</span>{finding.location}</p></div><span className="text-xs font-semibold text-muted-foreground md:block">{finding.source}</span><span><StatusBadge value={finding.severity} kind="severity" /></span><span className="font-mono text-xs">{finding.records.toLocaleString('es-ES')}</span><span><StatusBadge value={finding.status} kind="status" /></span><ChevronRight size={16} className="hidden text-muted-foreground md:block" /></button>)}</div>}
