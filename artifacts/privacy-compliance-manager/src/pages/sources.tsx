...
function SourceRow({ source }: { source: DataSource }) {
  ...
  return <div className="group grid gap-4 border-b border-border/70 px-5 py-5 last:border-0 md:grid-cols-[1.55fr_.8fr_.75fr_.72fr_.8fr_110px] md:items-center" data-testid={`row-source-${source.id}`}>
    ...
    <button disabled={scan.isPending} onClick={scanSource} className="inline-flex h-9 items-center justify-center gap-1.5 rounded-lg border border-border bg-background px-3 text-xs font-bold text-muted-foreground transition-colors hover:border-primary hover:text-primary disabled:cursor-wait disabled:opacity-60" data-testid={`button-scan-source-${source.id}`}>{scan.isPending ? <LoaderCircle size={14} className="animate-spin" /> : <Play size={13} />} Escanear</button>
  </div>;
}
...
export default function SourcesPage() {
  ...
  return <section className="page-in mx-auto max-w-[1440px] px-5 py-8 md:px-9 md:py-10">
    ...
    <div className="mb-6 grid gap-4 sm:grid-cols-3"><div className="rounded-2xl border border-card-border bg-card p-5 shadow-[var(--shadow-card)]"><p className="label-caps">Cobertura total</p><p className="mt-2 font-display text-2xl font-bold">100%</p><p className="mt-1 text-xs text-muted-foreground">fuentes con escaneo reciente</p></div><div className="rounded-2xl border border-card-border bg-card p-5 shadow-[var(--shadow-card)]"><p className="label-caps">Registros bajo control</p><p className="mt-2 font-display text-2xl font-bold">{(sourcesQuery.data ?? []).reduce((sum, source) => sum + source.records, 0).toLocaleString('es-ES')}</p><p className="mt-1 text-xs text-muted-foreground">entre todos los entornos</p></div><div className="rounded-2xl border border-card-border bg-card p-5 shadow-[var(--shadow-card)]"><p className="label-caps">Última actualización</p><p className="mt-2 font-display text-2xl font-bold">En vivo</p><p className="mt-1 flex items-center gap-1.5 text-xs text-primary"><RefreshCw size={12} /> sincronización activa</p></div></div>
    {sourcesQuery.isLoading ? <div className="rounded-2xl border border-card-border bg-card p-5">{[1, 2, 3].map((row) => <div className="flex gap-3 border-b border-border py-5" key={row}><div className="skeleton h-10 w-10 rounded-xl" /><div className="flex-1"><div className="skeleton h-4 w-1/3 rounded" /><div className="mt-2 skeleton h-3 w-1/4 rounded" /></div></div>)}</div> : sourcesQuery.isError ? <div className="rounded-2xl border border-[#edc5bd] bg-[#fff8f6] p-10 text-center"><p className="font-display font-bold">No pudimos conectar con las fuentes.</p><button onClick={() => sourcesQuery.refetch()} className="mt-4 text-xs font-bold text-primary underline" data-testid="button-retry-sources">Reintentar</button></div> : sourcesQuery.data?.length ? <div className="overflow-hidden rounded-2xl border border-card-border bg-card shadow-[var(--shadow-card)]"><div className="hidden grid-cols-[1.55fr_.8fr_.75fr_.72fr_.8fr_110px] gap-4 border-b border-border bg-[#f8fafb] px-5 py-3 font-mono text-[10px] uppercase tracking-[0.12em] text-muted-foreground md:grid"><span>Fuente</span><span>Entorno</span><span>Estado</span><span>Registros</span><span>Hallazgos</span><span /></div>{sourcesQuery.data.map((source) => <SourceRow key={source.id} source={source} />)}</div> : <div className="rounded-2xl border border-dashed border-border bg-card px-6 py-16 text-center"><ShieldCheck className="mx-auto mb-3 text-muted-foreground/50" size={30} /><p className="font-display text-lg font-bold">Aún no hay fuentes conectadas</p><p className="mt-1 text-sm text-muted-foreground">Cuando añadas una conexión aparecerá aquí su cobertura.</p></div>}
  </section>;
}