import { Bell, ChevronDown, Command, Database, FileBarChart2, Fingerprint, Gauge, Layers3, Menu, Radar, Search, ShieldCheck, Sparkles, X } from 'lucide-react';
            ...
            {navItems.map(({ href, label, icon: Icon }) => {
              ...
              return (
                <Link key={href} href={href} onClick={() => setMobileOpen(false)} className={`group flex items-center gap-3 rounded-xl px-3 py-2.5 text-[13px] font-semibold transition-colors ${active ? 'bg-sidebar-accent text-sidebar-accent-foreground' : 'text-sidebar-foreground/66 hover:bg-sidebar-accent/65 hover:text-sidebar-accent-foreground'}`} data-testid={`link-nav-${label.toLowerCase()}`}>
                  ...
                  <span>{label}</span>
                  {label === 'Hallazgos' && <span className="ml-auto rounded-md bg-[#efb34f]/15 px-1.5 py-0.5 font-mono text-[10px] text-[#f5c66f]">14</span>}
                </Link>
              );
            })}
          </nav>
        ...
        <div className="mt-auto px-4 pb-5">
          <div className="mb-4 rounded-2xl border border-sidebar-border bg-sidebar-accent/55 p-4">
            <div className="mb-3 flex items-center justify-between">
              <span className="font-mono text-[10px] uppercase tracking-[0.14em] text-sidebar-foreground/45">Cobertura</span>
              <span className="text-xs font-semibold text-sidebar-primary">Activa</span>
            </div>
            <div className="mb-2 flex items-end justify-between">
              <span className="font-display text-2xl font-bold text-white">92.4%</span>
              <span className="font-mono text-[10px] text-sidebar-foreground/45">últimos 30 días</span>
            </div>
            <div className="h-1.5 overflow-hidden rounded-full bg-sidebar-border"><div className="h-full w-[92.4%] rounded-full bg-sidebar-primary" /></div>
          </div>
          <div className="flex items-center gap-3 rounded-xl px-2 py-2">
            <span className="grid h-8 w-8 place-items-center rounded-full bg-[#d9e6e5] font-display text-xs font-bold text-[#214d50]">LM</span>
            <div className="min-w-0 flex-1">
              <p className="truncate text-xs font-bold text-white">Lucía Mena</p>
              <p className="truncate text-[11px] text-sidebar-foreground/45">Security lead</p>
            </div>
